...
Outputs:
A) Paper metrics (primary): per-joint MAE, RMSE, R^2 (unscaled angle units)
B) Per-test-file plots for selected joints (by mocap column name):
   1) ROM-normalized score (%) over time + polar (denominator: full TEST max-min ROM)
      Intended as visualization metric and matches your prior high percentages.
   2) IQR-normalized score (%) over time + polar (denominator: TRAIN IQR)
      More robust, often lower, useful as a stricter normalized view.

Both plot families compare predicted angles vs ground-truth angles (unscaled).
"""
...
# output_dir = "AA_Yan_mocap_included_0120/model6_0129_test_Vero/model_0130_IQR_ROM"
...
# AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero
output_dir = 'AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero/model_0130_IQR_ROM'
train_sensor_dir = "AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero/sensor_train"  # contains *_CapacitanceTable.csv
train_mocap_dir = "AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero/aa_train"      # contains *_resamp.csv
test_sensor_dir = "AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero/sensor_test"
test_mocap_dir = "AA_Yan_mocap_included_0120/model13_Teresa_Yan_to_Vero/aa_test"
...
# Joints included in time and polar plots
JOINTS_FOR_PLOTS = [
    "waist_yaw_joint",
    "waist_pitch_joint",
    "waist_roll_joint",
...
IQR_SCORE_YLIM = (0, 100)  # set to (60,100) only if you really want
...
def train_and_evaluate():
    ...
    # ---------------------------
    # Movement classifier (optional, kept)
    # ---------------------------
    ...
    # Denominators for both plot types
    # ROM from FULL TEST SET (visualization metric)
    full_true_sel = y_test_full[:, joint_idx]
    rom_test = full_true_sel.max(axis=0) - full_true_sel.min(axis=0)
    rom_test[rom_test == 0] = 1e-6

    # IQR from TRAIN set (stricter normalized metric)
    ...
    for fid in sorted(indices_by_file.keys()):
        ...
        # ---------------------------
        # 1) ROM-normalized score (test max-min)
        # ---------------------------
        norm_abs_err_rom = abs_err / rom_test
        norm_err_frame_rom = norm_abs_err_rom.mean(axis=1)
        rom_score_raw = 100.0 * (1.0 - norm_err_frame_rom)
        rom_score_raw = np.clip(rom_score_raw, 0.0, 100.0)
        ...
        mean_norm_rom_joint = norm_abs_err_rom.mean(axis=0)
        rom_joint_score = 100.0 * (1.0 - mean_norm_rom_joint)
        rom_joint_score = np.clip(rom_joint_score, 0.0, 100.0)
        ...
        # Save per-frame table (both scores + raw errors)
        ...
        # Save per-joint table (both scores + MAE/RMSE)
    ...
    with open(os.path.join(output_dir, "metric_definitions.txt"), "w") as f:
        f.write(
            "Visualization metric 1 (ROM-normalized score):\n"
            ...
            "Normalized metric 2 (IQR-normalized score):\n"
            ...
            "MAE and RMSE are computed in unscaled angle units.\n"
        )
    ...
    meta = {
        ...
        "notes": {
            "rom_norm": "uses full test-set ROM (max-min), intended for visualization",
    ...
    print(f"[Saved] metric_definitions.txt -> {output_dir}")
