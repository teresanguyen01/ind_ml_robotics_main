...
IMPORTANT CHANGE:
- This script EXCLUDES quaternion columns qw/qx/qy/qz from the mocap targets.
  It will NOT train models for those columns and will NOT report metrics for them.

Outputs:
- paper_tables/test_per_joint_metrics.csv
- feature_analysis/raw_feature_importance_all.csv
...
# CONFIG
# ---------------------------
# AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7
output_dir = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/models"
train_sensor_dir = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/sensor_train"
train_mocap_dir  = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/aa_train"
test_sensor_dir  = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/sensor_test"
test_mocap_dir   = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/aa_test"

plot_dir = "AA_Yan_mocap_included_0120/model27_toddler_train1_6_test7/normalized_sensor_plt"
...
# Mocap columns to EXCLUDE from targets
EXCLUDE_MOCAP_COLUMNS = {"left_hip_roll_joint", "left_hip_yaw_joint", "left_hip_pitch_joint", "right_hip_roll_joint", "right_hip_pitch_joint", "right_hip_yaw_joint", "qw", "qx", "qy", "qz"}
...
def load_mocap_df(file_path, handle_outliers=True, lower_q=0.01, upper_q=0.99, drop_first_n_cols=4) -> pd.DataFrame:
    ...
    # Drop quaternion targets if present
    drop_cols = [c for c in df.columns if c in EXCLUDE_MOCAP_COLUMNS]
    if drop_cols:
...
def plot_normalized_sensors_one_image(
    df: pd.DataFrame,
    key: str,
    out_dir: str,
    max_sensors: int = 24,
    ...
    """
    Plots up to `max_sensors` sensor columns as 24 separate subplots
    in a single image and saves as PNG.
    Assumes df is already normalized.
    """
    ...
    cols = list(df.columns)[:max_sensors]
    if not cols:
        return
    ...
    for i, col in enumerate(cols):
        axes[i].plot(x, df[col].to_numpy(dtype=float), linewidth=1.0)
        axes[i].set_title(str(col), fontsize=9)
        axes[i].grid(True, alpha=0.3)

    # Hide unused subplots if fewer than 24
    for j in range(n_sensors, len(axes)):
        axes[j].axis("off")

    fig.suptitle(f"Normalized sensor signals: {key}", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.98])

    out_path = os.path.join(out_dir, f"{key}_normalized_sensors.png")
    plt.savefig(out_path, dpi=200)
    plt.close(fig)



def load_dataset(sensor_dir: str, mocap_dir: str, strict_row_match=True, plot_dir: str = None):
    ...
    for s_path, m_path, key in pairs:
        ...
        if plot_dir is not None: 
            plot_normalized_sensors_one_image(X_df, key, plot_dir)
        y_df = load_mocap_df(str(m_path), handle_outliers=HANDLE_OUTLIERS, lower_q=LOWER_Q, upper_q=UPPER_Q, drop_first_n_cols=4)

        if y_df.shape[1] == 0:
            raise RuntimeError(
                f"[{key}] After excluding {sorted(EXCLUDE_MOCAP_COLUMNS)}, mocap has 0 columns. "
                "Check column names or exclude list."
            )
...
def booster_importance_to_df(booster: xgb.Booster, feature_names, importance_type: str, joint_name: str):
    """
    Returns a DataFrame with columns: joint, feature, importance_type, score
    """
    score_map = booster.get_score(importance_type=importance_type)  # keys like "f12"
    rows = []
...
def normalize_group_sum(df: pd.DataFrame, group_cols, score_col: str = "score"):
    """
...
def train_and_analyze():
    ...
    X_train, y_train, sensor_cols, mocap_cols, train_keys = load_dataset(
        train_sensor_dir, train_mocap_dir, strict_row_match=STRICT_ROW_MATCH, plot_dir=plot_dir
    )
    X_test, y_test, sensor_cols_t, mocap_cols_t, test_keys = load_dataset(
        test_sensor_dir, test_mocap_dir, strict_row_match=STRICT_ROW_MATCH, plot_dir=plot_dir
    )
    ...
    print(f"[INFO] Mocap targets (joint-only) count: {len(mocap_cols)}")
    ...
    for i in range(D):
        ...
        print(f"[INFO] Training target {i+1}/{D}: {joint_name}")
    ...
    # ---------------------------
    # Feature importance analysis (built-in gain/weight/cover)
    # ---------------------------
    if imp_rows:
        df_imp_all = pd.concat(imp_rows, ignore_index=True)
        df_imp_all.to_csv(os.path.join(feat_dir, "raw_feature_importance_all.csv"), index=False)

        df_gain = df_imp_all[df_imp_all["importance_type"] == "gain"].copy()
        if df_gain.empty:
            print("[WARN] No gain importances found. Possibly the model did not split on features.")
        else:
            ...
            df_global.to_csv(os.path.join(feat_dir, "global_feature_importance_gain.csv"), index=False)
            ...
            df_per_joint_top.to_csv(os.path.join(feat_dir, "per_joint_top_features_gain.csv"), index=False)
            ...
            df_feat_to_joints.to_csv(os.path.join(feat_dir, "feature_to_best_joints_gain.csv"), index=False)

            print(f"[Saved] Feature analysis CSVs -> {feat_dir}")

            # ---------------------------
            # Optional permutation importance (global, averaged over joints)
            # ---------------------------
            if DO_PERMUTATION_IMPORTANCE:
                print("[INFO] Computing permutation importance on a subset of test frames...")

                # Choose a subset of frames for speed
                ...
                preds_scaled = [m.predict(dmat_base) for m in models]
                Y_base_scaled = np.column_stack(preds_scaled)
                Y_base = scaler_y.inverse_transform(Y_base_scaled)
                base_mae = float(np.abs(Y_base - yt).mean())

                # Restrict to top features by global gain
                ...
                df_perm.to_csv(os.path.join(feat_dir, "permutation_importance_global.csv"), index=False)
                print(f"[Saved] permutation_importance_global.csv -> {feat_dir}")

    else:
        print("[WARN] No feature importance rows were collected. raw_feature_importance_all.csv will not be created.")
    ...
    meta = {
        ...
        "excluded_mocap_columns": sorted(list(EXCLUDE_MOCAP_COLUMNS)),
        ...
        "feature_analysis_outputs": {
            "raw_feature_importance_all": "feature_analysis/raw_feature_importance_all.csv",
            "global_feature_importance_gain": "feature_analysis/global_feature_importance_gain.csv",
            "per_joint_top_features_gain": "feature_analysis/per_joint_top_features_gain.csv",
            "feature_to_best_joints_gain": "feature_analysis/feature_to_best_joints_gain.csv",
            "permutation_importance_global": "feature_analysis/permutation_importance_global.csv" if DO_PERMUTATION_IMPORTANCE else None,
        },
        "notes": {
            "targets": "Joint angles only; qw/qx/qy/qz excluded",
            "feature_importance": "Built-in XGBoost importance exported; optional permutation importance exported if enabled.",
