...
# CONFIG (edit these)
# ---------------------------
#AA_Yan_mocap_included_0120/model9_0129_combos
output_dir = 'AA_Yan_mocap_included_0120/model9_0129_combos/model'
train_sensor_dir = "AA_Yan_mocap_included_0120/model9_0129_combos/sensor_train"  # contains *_CapacitanceTable.csv
train_mocap_dir = "AA_Yan_mocap_included_0120/model9_0129_combos/aa_train"      # contains *_resamp.csv
test_sensor_dir = "AA_Yan_mocap_included_0120/model9_0129_combos/sensor_test"
test_mocap_dir = "AA_Yan_mocap_included_0120/model9_0129_combos/aa_test"
...
def add_lags(X, lags=10):
    ...
    (Disabled by default in train_and_evaluate for now.)
    """
    T, F = X.shape
    out = [X]
...
def _basename_without_suffix(p: Path, suffix: str):
    stem = p.stem
    if stem.endswith(suffix): # Edited here 01/29
...
def match_pairs_strict(sensor_dir: str, mocap_dir: str):
    ...
    mocap_files = [p for p in mocap_files if p.stem.endswith(MOCAP_SUFFIX)] # edited here 01/29
...
def movement_from_key(key: str) -> str:
    ...
    Example movements you mentioned:
        Shoulder roll
        Hip twist
        Bicep curl
        Air punches
...
def load_dataset_from_dirs(sensor_dir: str, mocap_dir: str,
    ...
    for s_path, m_path, key in pairs:
        X = load_and_clean_csv(str(s_path),
                               handle_outliers=handle_outliers,
                               lower_q=lower_q, upper_q=upper_q,
                               drop_first_n_cols=0)
        y = load_and_clean_csv(str(m_path),
                               handle_outliers=handle_outliers,
                               lower_q=lower_q, upper_q=upper_q,
                               drop_first_n_cols=4)
...
def train_and_evaluate():
    ...
    # Load train/test (with movement + file IDs)
    ...
    # 1) Drop 'Unknown' labels – we do not train or evaluate classifier on them
    ...
    # 2) Encode ONLY known labels, so ids are 0..C-1
    le = LabelEncoder()
    ...
    # Optional: warn if any labels appear only in TEST
    train_label_set = set(train_mov_known)
    test_label_set  = set(test_mov_known)
    only_in_test = sorted(test_label_set - train_label_set)
    if only_in_test:
        print("[WARN] The following movement labels appear only in TEST; "
              "the classifier has never seen them during training:", only_in_test)
    ...
    clf.fit(
        X_train_cls,
        y_cls_train,                     # encoded ints 0..C-1
    ...
    # Time-series comparison plots (all dims in a grid)
    cols = 4
    ...
    for dim in range(num_dims):
        ...
        ax.set_title(f'Dim {dim+1} (R² = {r2_val:.3f})')
        ax.set_ylabel('Value')
        ax.legend()
    ...
    plt.close()
    print(f"[Saved] Combined time-series plot: {plot_path}")

    # Per-dimension MAE bar plot
    ...
    plt.tight_layout()
    mae_path = os.path.join(plots_dir, "mae_per_dim.png")
    plt.savefig(mae_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[Saved] MAE per dimension plot: {mae_path}")
